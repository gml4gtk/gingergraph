/* $Id: Directory.c,v 1.2 1996/01/30 16:12:18 lupus Exp $ */

/***************************************************************
 * This file is originally part of the Free Widget Foundation  *
 * widget set.	        				       *
 * It is slightly modified for use in the Ginger project by    *
 * Niels C. Larsen, Aalborg University, Denmark                *
 * (lupus@iesd.auc.dk)                                         *
 * File first adapted for Ginger 6-1-1996                      *
 ***************************************************************/
...
char *old_path,*new_path;
{
	...
	if (getwd(path) == 0) return(NULL);
	if (chdir(old_path) != 0) return(NULL);
	if (getwd(new_path) == 0) strcpy(new_path,old_path);
